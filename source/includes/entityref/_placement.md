...
| bteSyncStatus | String (100) | Pay and Bill corporations only: Allows for viewing whether a Bullhorn placement was synced to BTE successfully, pending, or if there was an issue. This field should only appear if a corporation is WFR enabled and PeopleNet Front Office Integration enabled. | X | X |
...
| jobLocation | To-one association | Location associated to Placement's JobOrder | | |
...
| location | To-one association | Location associated to Placement | | |
...
| owner | To-one association | CorporateUser who owns the JobOrder. The default value is user who creates the JobOrder and is also the default owner of Placement records created from this job. The field map name shown in ATS for owner will be jobUserID. | | |
